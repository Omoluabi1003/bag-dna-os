...
import { detectThreatPatterns } from "@/lib/intelligence";
import { ThreatGraphView } from "@/components/intelligence/ThreatGraphView";

export default function Page() {
  const data = detectThreatPatterns();
  return (
    <AppShell title="Threat Graph™" eyebrow="Security Flow · threat signatures">
      <section className="mb-6 rounded-[28px] border border-white/[.08] bg-white/[.035] p-6 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-3 max-w-3xl font-display text-3xl font-semibold tracking-[-.03em] text-ivory">Threat signatures, severity, historical similarity, evidence, and recommendations translate anomalies into security decisions.</h2>
          </div>
          <Badge tone="emerald">Seeded Intelligence</Badge>
        </div>
      </section>
      <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
        <ThreatGraphView threat={data} />
        <section className="glass p-5">
          ...
          <Link href="/custody" className="mt-5 inline-flex rounded-xl bg-cyan px-4 py-3 text-xs font-black text-ink">Open custody chain →</Link>
        </section>
      </div>
