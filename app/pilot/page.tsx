...
export default function PilotPage() {
  return <AppShell title="Global Pilot & Standards Review" eyebrow="ICAO-aligned evaluation pathway">
    <section className="glass p-6 md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div className="max-w-3xl">
          <Badge tone="gold"><Globe2 size={12}/> International review posture</Badge>
          <h2 className="mt-5 text-4xl font-semibold tracking-tight md:text-6xl">Prepared for ICAO-aligned stakeholder evaluation.</h2>
        ...
        <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/[.07] p-5 text-right">
          ...
          <p className="mt-2 text-2xl font-semibold">Prepared</p>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <section className="glass p-6">
        ...
        <div className="space-y-3">{phases.map(([n,title,text],i)=><div className="grid grid-cols-[42px_1fr_auto] gap-4 rounded-2xl border border-white/[.08] bg-white/[.02] p-4" key={n}><span className="font-display text-2xl text-gold">{n}</span><div><h3 className="text-sm font-bold">{title}</h3><p className="mt-2 text-[11px] leading-5 text-mist">{text}</p></div><Badge tone={i===0?"emerald":i===4?"gold":"slate"}>{i===0?"Ready":i===4?"Review":"Planned"}</Badge></div>)}</div>
      </section>
