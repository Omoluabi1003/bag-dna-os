...
import type { LiveOperationsSnapshot } from "@/lib/integrations/liveOperations";
import { Badge, StatusDot } from "@/components/ui";

const corridor = { minLon: -84, maxLon: -78, minLat: 24.2, maxLat: 35.5 };
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[28px] border border-cyan/15 bg-[#06111d] shadow-[0_30px_90px_rgba(0,0,0,.32)]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[.08] bg-white/[.025] px-5 py-4 md:px-7">
          ...
          <div className="flex flex-wrap gap-2"><Badge tone={snapshot.mode === "live" ? "emerald" : "amber"}><StatusDot/> {snapshot.mode === "live" ? "Feeds live" : "Continuity mode"}</Badge><Badge>Updated {new Date(snapshot.generatedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit", timeZone: "UTC" })} UTC</Badge></div>
        </div>
        ...
        <div className="grid lg:grid-cols-[1.4fr_.8fr]">
          <div className="relative min-h-[440px] overflow-hidden border-r border-white/[.08] bg-[radial-gradient(circle_at_35%_40%,rgba(46,211,183,.08),transparent_24%),linear-gradient(rgba(255,255,255,.025)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.025)_1px,transparent_1px)] bg-[size:auto,36px_36px,36px_36px] p-6">
            <div className="relative z-20 flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-cyan">Regional air picture</p><h3 className="mt-2 text-xl font-semibold text-white">Anonymous ADS-B aircraft positions</h3></div><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-[9px] font-bold text-emerald-300">AUTO REFRESH · 60S</span></div>
            <div className="absolute inset-x-6 bottom-6 top-24 rounded-2xl border border-white/[.06] bg-[#06101a]/65">
              ...
              {!snapshot.aircraft.length && <div className="absolute inset-0 grid place-items-center text-center"><div><RefreshCw className="mx-auto mb-3 text-amber-300"/><p className="text-sm font-semibold text-white">Aircraft feed temporarily unavailable</p><p className="mt-1 text-[10px] text-slate-400">The console will retry automatically.</p></div></div>}
            </div>
          </div>
          ...
          <div className="p-5 md:p-6">
            ...
            <div className="mt-5 space-y-3">{snapshot.airportWeather.map((weather) => <article key={weather.station} className="rounded-2xl border border-white/[.07] bg-white/[.025] p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><MapPin size={13} className="text-gold"/><b className="font-mono text-sm text-white">{weather.station}</b></div><Badge tone={weather.flightCategory === "VFR" ? "emerald" : weather.flightCategory === "MVFR" ? "cyan" : "amber"}>{weather.flightCategory}</Badge></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div><p className="text-[8px] uppercase text-slate-500">Temp</p><b className="mt-1 block text-sm text-white">{weather.temperatureC ?? "—"}°C</b></div><div><p className="text-[8px] uppercase text-slate-500">Wind</p><b className="mt-1 block text-sm text-white">{weather.windSpeedKt ?? "—"} kt</b></div><div><p className="text-[8px] uppercase text-slate-500">Visibility</p><b className="mt-1 block text-sm text-white">{weather.visibilityMi ?? "—"} mi</b></div></div><p className="mt-3 truncate font-mono text-[8px] text-slate-500">{weather.raw}</p><p className="mt-2 text-right text-[8px] text-slate-600">Observed {formatObservedAt(weather.observedAt)}</p></article>)}
