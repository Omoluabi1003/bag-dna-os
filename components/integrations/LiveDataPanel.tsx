  ...
  return (
    <section className="glass p-6" aria-live="polite">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge tone={state.mode === "live" ? "emerald" : "amber"}>{state.mode === "live" ? "Live API mode" : "Beta data mode · degraded"}</Badge>
      </div>
      {state.mode === "degraded" && <p className="mt-4 flex gap-2 rounded-xl border border-amber-400/20 bg-amber-400/10 p-3 text-[11px] text-amber-200"><TriangleAlert size={15}/> One or more public APIs were unavailable. Safe fallback data is active.</p>}
      ...
      <button onClick={() => void load()} className="mt-4 rounded-xl border border-white/10 px-4 py-2 text-[11px] font-semibold text-cyan hover:bg-white/5">Refresh public context</button>
    </section>
  );
}
