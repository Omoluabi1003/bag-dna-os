...
export interface BagIntegrityScore { score: number; band: "Verified" | "Monitor" | "Elevated" | "Investigate" | "Critical"; positiveSignals: string[]; deductions: string[]; }
export interface VerificationResult { bagDnaId: string; score: number; decision: "pass" | "monitor" | "investigate" | "fail"; checks: Record<string, boolean>; reasons: string[]; recommendedAction: string; integrity: BagIntegrityScore; }
export interface ClaimVerification { bagDnaId: string; passengerClaimToken: string; boardingPassRef: string; identityMatch: boolean; integrityScore: number; sealStatus: SealStatus; finalCustodyEvent: CustodyEvent; custodyClosed: boolean; verifiedAt: string; }
export interface DigitalBaggageIdentity { bagDnaId: string; passenger: PassengerIdentityRef; flight: FlightRef; physicalProfile: BagPhysicalProfile; tag: TagToken; rfid: RfidCredential; nfc: NfcCredential; qr: RotatingQrCredential; seal: TamperSeal; fingerprint: VisualFingerprint; custodyEvents: CustodyEvent[]; riskScore: number; integrityScore?: BagIntegrityScore; ledger: EvidenceLedgerEvent[]; }
